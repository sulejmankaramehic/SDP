...
Flight::route('POST /appointments/remove', function(){
  Flight::json(Flight::jwt(Flight::appointmentsService()->remove(Flight::request()->data->getData())));
});
...
Flight::route('GET /appointments', function(){
  ...
  $search = Flight::query('search');
  $order = Flight::query('order', "-id");
  Flight::json(Flight::appointmentsService()->get_appointments($search, $offset, $limit, $order));
});
?>
